...
The left panel keeps a compact guided start strip visible above Assets, Layouts, and Templates. The flow keeps beginner work in a short path: choose a template, add or replace an image, add a title, apply brand styling, then generate/export layout text. Template filters use `All`, `YouTube`, `Shorts`, `Stream`, `Cutout`, and `Schedule` labels so creators can quickly narrow the 26 bundled starts, including six schedule starts for yearly, monthly, and daily layouts.
