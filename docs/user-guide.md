...
- Manual: use the footer Manual button to open a fixed-size reference modal. The left tabs choose feature areas, with Overview at the top for what the app can do, the feature list, and the basic workflow. The top tabs choose sections, and the right-side contents jump to the selected entry top while highlighting the matching manual entry on focus or hover. Each feature entry includes two compact real GUI captures for access and operation; click a capture to open a larger image-only view. The shortcut section shows separate SVG diagrams for each keyboard and mouse operation. The modal follows the active Japanese/English GUI labels, adds a use-case line to each feature, explains parameter and dropdown item effects, documents call paths such as Output menu to OBS preview, Colors to image extraction, and multi-selection to relative editing, and remembers the last tabs and scroll position after closing.
...
Open Templates and choose one of the generator buttons. **Generate schedule** creates monthly, weekly, or daily editable schedule objects. **Standard thumbnail**, **Vertical thumbnail**, and **Stream waiting** create editable image layouts. The previous bundled default-template list is no longer shown in the current GUI. Registered templates remain separate named snapshots saved in localStorage.
...
Use **Generate schedule** to choose monthly, weekly, or daily schedule, canvas orientation, month/date from the browser calendar-style input, Sunday or Monday week start where relevant, weekday language, date format, title, the same font list used by Adjust, title/weekday/date/plan size sliders, grid style, corner radius, line width, colors, and whether generated objects should be grouped. Daily schedules add compact AM/PM period labels, horizontal start/end time inputs, AM/PM event display toggles, optional AM `0`-`11` and PM `12`-`23` clock-hour labels, circle size, circle spacing, and event labels, then generate editable AM/PM circle layouts with sector objects. In Day mode, grid style and corner radius are replaced by circle size and circle spacing controls, and portrait circles are centered horizontally. The sector starts and ends are synchronized to clock positions, so `09:00` starts from the left side of the circle. Schedule landscape preview is widened on desktop, and portrait preview keeps the generated portrait ratio. Use **Standard thumbnail** for a general 1280x720 video thumbnail and **Vertical thumbnail** for a 1080x1920 short-form thumbnail. Use **Stream waiting** when you need an animated waiting screen object set for アニメ/OBS preview. Each image generator has five placement patterns. Its Grid / text controls are grouped into Common, Title, Subtitle, and Label sections covering font family/weight, letter spacing, common corner radius, font sizes, stroke widths, and text alignment; desktop modals place paired sliders side by side to reduce height. Letter spacing can be set to `0` from the number field or slider. The Tone selector updates the preview for all three image generators, while the animation checkbox is shown only for Stream waiting. The modals also provide content, color, image-slot, grouping, preview, **Generate objects**, and **Save settings** controls. **Generate objects** saves the generator settings before replacing the current canvas object list, updating canvas size where needed, and refreshing the internal CSV/HTML layout text used by edit state and templates. **Save settings** stores only the modal settings so they are restored after reload without changing the current canvas.

## OBS Preview

Open OBS preview from the preview-header Output menu. The preview opens as a separate popup-style window and renders only the current animated canvas, without editor panels, selection boxes, or resize handles. In Japanese mode, the child-window controls use `一時停止`, `リセット`, and `非表示`; in English mode they use Play/Pause, Reset, and Hide. Press `P` to play or pause, `R` to reset, `H` to hide or show the controls, and `F` or Enter to retry fullscreen. OBS should capture the browser window or fullscreen view; whether browser chrome is removed depends on browser permissions and OBS capture settings.
...
Shape objects support Rect, Ellipse, Triangle, Diamond, Pentagon, Hexagon, Star, Sector, and Line. Corner radius rounds Rect and polygon shape corners; Sector uses clock-style Start angle, End angle, and Inner radius controls; Line remains controlled by stroke width and line style.
