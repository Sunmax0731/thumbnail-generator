...
The Assets tab stays focused on the imported media list. Imported assets are selectable rows with direct actions for adding an image layer, opening the asset in Image Lab, or deleting the asset. Deleting an asset also removes image layers that reference it. Local image import sits in the preview-pane Output section, left of the export buttons, and the YouTube URL import UI is hidden while browser-only helper code remains available.
...
The Templates tab separates bundled default templates and Browser templates as named reusable snapshots. Both template lists can be resized independently. Loading a template asks for confirmation before replacing the current canvas and automatically applies the template's output aspect ratio. Edit state and export actions live in the preview pane so save, restore, JSON backup, autosave, image import, and file output controls stay visible beside the current thumbnail instead of being hidden behind a left-panel tab. Export and Edit state sit side by side in the bottom preview pane on desktop, and each section stacks its own action buttons vertically for predictable scanning.
