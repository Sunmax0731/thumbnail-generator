...
- Configure one or more per-layer animation sets with easings.net-style easing choices, motion presets, selected-object Motion preview, easing graph, disabled direction controls when an animation type does not use movement, a bottom timeline of animated layers, text-only motion controls, glow/effect motion controls, and a popup-style OBS preview window without editor controls or selection handles.
- Keep Motion common parameters, text-only motion, and glow/effect motion in separate collapsible sections so advanced animation controls can expand without crowding the inspector.
- Provide OBS preview-window controls for Play/Pause, Reset, and hiding/showing the control overlay for clean capture.
