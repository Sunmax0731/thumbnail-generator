...
Latest validation note: 2026-06-12 Generator rollout passed `npm test` (30 files, 117 tests), `npm run build`, and a Playwright headless Chromium runtime gate at `http://127.0.0.1:4327/thumbnail-generator/`. The gate confirmed four generator buttons, no visible default-template list, schedule settings persistence, video thumbnail vertical-variant persistence after reload, nonblank generated canvases, image import, canvas drag, WebP export, mobile modal rendering, mobile horizontal overflow `0`, and no page/console errors. GitHub Pages deploys through `.github/workflows/pages.yml` on pushes to `codex/thumbnail-generator-static-app`.
