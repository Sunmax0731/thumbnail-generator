...
Latest validation note: 2026-06-12 schedule generator button height fix passed `npm test` (30 files, 120 tests), `npm run build`, and a Playwright headless Chromium runtime check at `http://127.0.0.1:4338/thumbnail-generator/`. The gate confirmed Schedule portrait action buttons remain directly below the preview and each measured `32px` high, with no relevant app console errors or non-analytics HTTP errors. GitHub Pages deploys through `.github/workflows/pages.yml` on pushes to `codex/thumbnail-generator-static-app`.
