...
- 上部ツールバー: 表示言語、プリセット、幅、高さ、形式、品質、PNG/JPG/WebP/出力を操作します。
...
テンプレートを読み込んだあと、上部ツールバーで出力プリセット、幅、高さ、形式、品質を確認します。YouTube 16:9 なら `1280x720`、Shorts 向けなら縦長プリセットを使います。
...
5. 必要に応じて **Image Lab を開く** から切り抜きやクロマキー処理を行います。

![Image Lab モーダル](assets/runtime-image-lab-modal.png)

Image Lab は画像処理用のモーダルです。画像を選び、クロマキー、矩形、円、ポリゴンの切り抜きを設定して、処理済み画像を新しい画像レイヤーとして追加できます。
...
テキストレイヤーを選択して **調整** を開くと、名前、位置、サイズ、回転、不透明度、ぼかし、文字列、文字サイズ、線、カーニング、フォント、組み方向、塗り、縁取り、文字揃えを編集できます。**文字を枠に合わせる** は、レイヤー範囲に収まる最大文字サイズを自動設定します。
...
図形レイヤーでは、図形種別、角丸、塗り、線、塗り透明度、線幅、線透明度を編集できます。バッジや区切り線は、サムネイル内の注目箇所や構造を作るために使います。
...
1. 上部ツールバーで形式を PNG、JPG、WebP から選びます。
2. 必要に応じて品質スライダーを調整します。
3. 個別の PNG/JPG/WebP ボタン、または **出力** ボタンを選びます。
4. ブラウザのダウンロードとしてサムネイル画像が保存されます。
