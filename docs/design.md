...
The Colors tab treats registered single colors as editable rows. Selecting a row loads it into the editor, Update writes it back, Fill/Stroke group rows apply or delete paired group colors, and saved multi-color palette rows stay visually separate from the single-color list. The palette maker shows an Adobe-style color exploration area with a drag-capable color wheel, generated-color points, large palette bars, HEX/RGB synchronized input, recent colors, and a full-width opacity control. Users can save the displayed pattern as a palette-level set, then apply any saved-palette color as Fill or Stroke without replacing the single-swatch workflow.
...
- Interaction: concept showed editor controls; implementation adds working CSV/HTML import, file import, YouTube thumbnail import, selected asset insertion, expanded quick add with line layers, default templates, layer edits, signed inner/outer edge blur with text/stroke control, corner radius, blank-click deselection, live multi-layer relative movement/rotation, layer grouping, individual grouped-row editing, fit selected layers to canvas, keyboard edit shortcuts, multi-layer angle matching, saved edit state with autosave, canvas selection, Image Lab modal processing with selected asset handoff and editable Rect/Circle/Polygon selections, text fit-to-box, text kerning, horizontal/vertical text writing mode, text alignment buttons, disabled inert inspector controls, Adjust reset buttons, editable single-color rows, deletable Fill/Stroke color groups, saved multi-color palette sets, drag-capable palette maker preview, resizable inspector lists, Google Fonts dropdown options, and PNG/JPEG/WebP export.
