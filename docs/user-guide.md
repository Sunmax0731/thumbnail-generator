...
Open Templates and choose a bundled default template to replace the current canvas with a complete starting layout. Confirm the localized dialog before the current layers and canvas size are replaced; the template's aspect ratio is applied automatically, and the preview zoom auto-fits so the new canvas is not clipped. The shipped templates cover 38 practical starts with five entries each for YouTube, Shorts, Stream, and Cutout categories, eight Schedule entries for yearly, monthly, weekly, and daily layouts, and ten animated eyecatch/waiting entries. Browser templates remain separate named snapshots saved in localStorage. Deleting a browser template also opens the same confirmation modal style used by other destructive actions. Drag the resize handles under Default templates or Browser templates to change how much vertical space each list uses.
