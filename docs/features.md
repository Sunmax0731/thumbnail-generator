...
- Generate beta monthly, weekly, or daily schedule templates from the Templates tab with calendar date input, pre-generation preview, per-day or uniform action counts where relevant, daily period/start-time/end-time/event labels, weekday language, date format, canvas orientation, grid or daily layout style, the same font choices as Adjust, separate title/weekday/date/plan size sliders, color, corner radius, line width, generated-object grouping settings, range-aware badge labels, and reload-persistent settings. Daily mode creates an AM/PM circle layout with editable sector shapes, compact Day-only controls, independent AM/PM event visibility toggles, AM `0`-`11` and PM `12`-`23` clock-hour labels, adjustable circle size and circle spacing, horizontally centered portrait circles, and sector start/end angles synchronized to the configured clock times. Schedule landscape previews are wider on desktop, and portrait previews keep the generated portrait ratio.
...
- Open the fixed-size in-app manual from the footer, then use left category tabs, top section tabs, and the right-side contents. The manual starts with an Overview category whose top tabs cover what the app can do, the feature list, and the basic workflow. It highlights the matching entry when a contents item is focused or hovered, scrolls a clicked contents item to the entry top, preserves the last viewed tabs and scroll position after closing, follows the active Japanese/English GUI labels, adds real access/operation GUI captures to each feature entry with click-to-enlarge viewing, adds operation-specific shortcut and mouse-operation SVG diagrams, adds use cases to each feature entry, and explains parameter/dropdown item effects plus call paths such as Output menu to OBS preview, Colors to image extraction, and multi-selection to relative editing.
...
- Choose Rect, Ellipse, Triangle, Diamond, Pentagon, Hexagon, Star, Sector, or Line for shape objects. Sector objects expose clock-style start angle, end angle, and inner radius controls.
