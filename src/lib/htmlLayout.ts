...
export function parseHtmlLayout(htmlText: string, options: LayoutParseOptions): LayoutParseResult {
  ...
  const layers: ThumbnailLayer[] = nodes.flatMap((node, index): ThumbnailLayer[] => {
    ...
    const base = {
      ...
      shadowColor: attr(node, "shadow-color") || "#000000",
      shadowOpacity: clamp(numberOr(attr(node, "shadow-opacity"), 0), 0, 1),
      shadowBlur: numberOr(attr(node, "shadow-blur"), 0),
      shadowDistance: numberOr(attr(node, "shadow-distance"), 0),
      shadowAngle: numberOr(attr(node, "shadow-angle"), 135),
      rotateX: numberOr(attr(node, "rotate-x"), 0),
      rotateY: numberOr(attr(node, "rotate-y"), 0),
      bevelSize: numberOr(attr(node, "bevel-size"), 0),
      bevelOpacity: clamp(numberOr(attr(node, "bevel-opacity"), 0), 0, 1),
