...
- Left rail: Assets and Templates. The previous Layouts tab is hidden from the GUI; CSV/HTML layout import and generation are handled in a collapsible preview-pane Layout I/O section.
- Right rail: Layers, Adjust, Colors, and Motion.
...
The Assets tab stays focused on media import. Imported assets are selectable rows with direct actions for adding an image layer or opening the asset in Image Lab. A compact YouTube URL field imports a video thumbnail into the same asset list and immediately creates an editable image layer.
...
The Templates tab separates bundled default templates and Browser templates as named reusable snapshots. Edit state and Layout I/O now live in the preview pane so save, restore, JSON backup, autosave, CSV/HTML generation, and CSV/HTML import controls stay visible beside the current thumbnail instead of being hidden behind a left-panel tab.
...
- Preset changes keep the current user zoom. Users can choose Fit canvas explicitly or pan the preview with Pan, Space-drag, or Alt-drag. When a layer is dragged outside the document, edit padding expands in pixels while the canvas frame uses the same user zoom scale, so the document does not shrink to fit the new overflow.
...
- Interaction: concept showed editor controls; implementation adds working preview-pane CSV/HTML import, file import, YouTube thumbnail import, selected asset insertion, collapsible Layers Quick Add with line layers, collapsible layer list, 38 bundled default templates including Schedule and Motion, preview-pane edit state, manual Fit canvas plus pan, layer edits, signed inner/outer edge blur with text/stroke control, polygon corner radius, blank-click deselection, live multi-layer relative movement/rotation, layer grouping, individual grouped-row editing, fit selected layers to canvas from Adjust, alignment with even distribution, keyboard edit shortcuts, multi-layer angle matching, saved edit state with autosave, canvas selection, Image Lab modal processing with selected asset handoff and editable Rect/Circle/Polygon selections, text fit-to-box, text kerning, horizontal/vertical text writing mode, text alignment buttons, disabled inert inspector controls, Adjust color buttons with shared palette picker, Motion animation controls with selected-object preview and easing graph, OBS preview window playback, editable single-color rows with direct Fill/Stroke apply buttons, collapsible saved multi-color palette sets, drag-capable palette maker preview, resizable inspector lists, expanded shape kinds, Google Fonts dropdown options, and PNG/JPEG/WebP export.
- Selection fidelity: vertical text uses the configured layer box for selection, hit testing, and resize handles so width/height edits behave like horizontal text, while preview padding still accounts for rendered vertical columns when content extends beyond the edit box.
