...
The Assets tab contains local image import, direct-folder image import, the imported media list, and reusable group objects. Importing images opens a tag-assignment modal before the rows are registered. Imported assets are selectable rows with direct actions for adding an image layer, opening the asset in Image Lab, deleting the asset, and adding/removing multiple tags. Group object rows mirror the image asset management pattern with reuse, deletion, and tag editing. The image section and group object section can each collapse and resize, and a shared tag filter narrows both lists. Rows keep minimum heights so dense imported lists do not compress row actions. Deleting an asset also removes image layers that reference it. The YouTube URL import UI is hidden while browser-only helper code remains available.
