...
- Add simple shapes with fill color, stroke color, size, position, rotation, and shape kinds for rectangle, ellipse, triangle, diamond, pentagon, hexagon, star, sector, and line.
...
- Provide a beta schedule generator in Templates that creates monthly, weekly, or daily editable schedule layer sets from calendar date inputs, Sunday/Monday week-start settings where relevant, weekday language, date format, uniform or per-day action counts where relevant, daily period/start-time/end-time/event labels, canvas orientation, grid or daily layout style, Adjust-shared font choices, separate title/weekday/date/plan font-size sliders, color, corner radius, line-width, pre-generation preview, range-aware badge labels, generated-layer grouping controls, and reload-persistent generator settings. The daily generator must support an AM/PM circle layout with editable sector shapes, compact start/end time controls, independent AM/PM event visibility, optional AM `0`-`11` and PM `12`-`23` clock-hour labels, adjustable circle size and circle spacing, horizontally centered portrait circles, clock-synchronized sector start/end angles, and a customizable date/title heading such as a selected day's schedule.
...
- Show a footer GitHub Issues path with a GitHub icon for bug reports and feature requests, and show the X contact with the official X logo.
- Show browser-only privacy/storage guidance inside the app without requiring a Templates service section.
- Publish a privacy policy and terms of use from the static app, and open localized privacy and terms content from taller footer modals that avoid desktop scrollbars where the viewport allows. Keep footer entries for Manual, privacy, terms, X contact, issue reporting, and centered copyright.
