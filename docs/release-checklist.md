...
Latest manual validation note: 2026-06-14 manual real-capture correction passed `npm test` (35 files, 141 tests), `npm run build`, and a Playwright local Chrome runtime check at `http://127.0.0.1:4401/thumbnail-generator/?runtime=manual-real-captures-20260614`. The gate confirmed the first Manual side tab is `Overview`, Overview top tabs are `What it does`, `Feature list`, and `Workflow`, feature entries show actual PNG access/operation GUI captures from `public/manual-captures`, feature entries no longer use `.manual-gui-svg` placeholders, shortcut/mouse entries show dedicated SVG diagrams only, layer editing works, WebP export works, mobile manual overflow is `0px`, and no page/app runtime errors were reported.
