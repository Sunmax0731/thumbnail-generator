...
- Use the Templates tab generator buttons to create editable monthly, weekly, or daily schedule layouts, standard thumbnails, vertical thumbnails, and stream waiting screens. Daily schedules can include AM/PM sector circles, compact start/end time inputs, independent AM/PM event visibility, AM `0`-`11` and PM `12`-`23` clock-hour labels, adjustable circle size, and adjustable circle spacing.
...
- Open the in-app manual from the bottom service footer; the larger manual starts with an Overview category for what the app can do, feature list, and workflow steps, follows the current Japanese/English language setting, uses the matching GUI labels for that language, adds real access/operation GUI captures to feature entries with click-to-enlarge image viewing, adds operation-specific keyboard and mouse SVG diagrams, adds a use case to each feature entry, explains parameter and dropdown item effects in bullets, and remembers the last category, section, and scroll position after closing.
