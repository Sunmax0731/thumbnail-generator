...
- Generate beta monthly or weekly schedule templates from the Templates tab with calendar date input, pre-generation preview, per-day or uniform action counts, weekday language, date format, canvas orientation, grid style, font size/style, color, corner radius, line width, and generated-layer grouping settings.
