...
- 上部ツールバー: 表示言語、プリセット、幅、高さを操作します。PNG/JPG/WebP の直接出力はプレビュー下部の編集状態セクション横にあります。
...
テンプレートを読み込んだあと、上部ツールバーで出力プリセット、幅、高さを確認します。YouTube 16:9 なら `1280x720`、Shorts 向けなら縦長プリセットを使います。形式はプレビュー下部の PNG/JPG/WebP の直接出力ボタンで選びます。
